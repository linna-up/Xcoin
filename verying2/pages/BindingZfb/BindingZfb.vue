<template>
	<view class="usdt" :style="{width:windowWidth+'px',height:windowHeight+'px'}">
		...
		<view class="text">{{i18n.BindingAlipay}}</view>
		<view class="addresstext">{{i18n.Name}}</view>
		<view class="address">
			<input type="text" v-model="username" :placeholder="i18n.AlipayName" />
		</view>
		<view class="addresstext">{{i18n.Account}}</view>
		<view class="address">
			<input type="text" v-model="account" :placeholder="i18n.AccountInput" />
		</view>
		<view class="texts">{{i18n.AddQr}}</view>
		<view class="add">
		...
		<view class="btm" @click="updata">{{i18n.Save}}</view>
	</view>
</template>
...
<script>
	...
	export default {
		...
		computed:{
			i18n(){
				return this.$t('message').mine.BindingZfb
			}
		},
		methods: {
			updata() {
				if (this.username === "" || this.username === null || this.username === undefined) {
					uni.showToast({
						title: this.$t('message').mine.BindingZfb.NameInput,
						icon: "none"
					})
					return;
				}else if (this.account === "" || this.account === null || this.account === undefined) {
					uni.showToast({
						title: this.$t('message').mine.BindingZfb.AlipayInput,
						icon: "none"
					})
					return;
				}
