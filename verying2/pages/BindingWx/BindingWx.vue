<template>
	<view class="usdt" :style="{width:windowWidth+'px',height:windowHeight+'px'}">
		...
		<view class="text">{{i18n.BindingWx}}</view>
		<view class="addresstext">{{i18n.Name}}</view>
		<view class="address">
			<input type="text" :placeholder="i18n.WxName" v-model="username"/>
		</view>
		<view class="addresstext">{{i18n.Account}}</view>
		<view class="address">
			<input type="text" :placeholder="i18n.AccountInput" v-model="account" />
		</view>
...
<script>
	...
	export default {
		...
		computed:{
			i18n(){
				return this.$t('message').mine.BindingWx
			}
		},
		methods: {
			updata() {
				if (this.username === "" || this.username === null || this.username === undefined) {
					uni.showToast({
						title: this.$t('message').mine.BindingWx.WxNameInput,
						icon: "none"
					})
					return;
				}else if (this.account === "" || this.account === null || this.account === undefined) {
					uni.showToast({
						title: this.$t('message').mine.BindingWx.WxAccountInput,
						icon: "none"
					})
					return;
				}
