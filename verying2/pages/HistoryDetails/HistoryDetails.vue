<template>
	<view class="details" :style="{width:windowWidth+'px',height:(windowHeight-top-45-50)+'px'}">
		...
		<view class="item">
			<view class="item_left">{{i18n.State}}</view>
			<view class="item_right">{{list.status_paid}}</view>
		</view>
		<view class="item">
			<view class="item_left">{{i18n.Address}}</view>
		...
		<view class="item">
			<view class="item_left">{{i18n.ServiceCharge}}</view>
		...
		computed: {
			status() {
				if (this.list.status === "1") {
					return this.$t('message').mine.HistoryDetails.Audit
				} else if (this.list.status === "2") {
					return this.$t('message').mine.HistoryDetails.Completed
				} else if (this.list.status === "3") {
					return this.$t('message').mine.HistoryDetails.Undone
				}
			},
			i18n(){
				return this.$t('message').mine.HistoryDetails
			}
		},
		methods: {
