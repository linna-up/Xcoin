<template>
	<view class="invi" :style="{width:windowWidth+'px',height:(windowHeight)+'px'}">
		<InviNav></InviNav>
		<view class="nav_text">{{i18n.Invite}}</view>
		<view class="nav_texts">{{i18n.InviteCode}}</view>
		<view class="nav_textss">{{address}}</view>
		<view class="qrCode">
			<image :src="img"></image>
		</view>
		<view class="btm">
			<view class="btm_item" @click="capture">{{i18n.Save}}</view>
			<view class="btm_item" @click="copy(address)">{{i18n.Copy}}</view>
		</view>
	</view>
</template>
 ...
 */
	...
	export default {
		...
		computed:{
			i18n(){
				return this.$t('message').mine.InviteFriends
			}
		},
		methods: {
			capture() {
				var i18n = this.$t('message').mine.InviteFriends
