<template>
	<view class="history" :style="{width:windowWidth+'px',height:(windowHeight-top-45-50)+'px'}">
		...
		<view class="history_title">{{i18n.History}}</view>
		<!-- <view class="title_type">快速提币</view> -->
		<view class="title_text">
			<view class="title_text_left">{{i18n.Number}}</view>
			<view class="title_text_center">{{i18n.State}}</view>
			<view class="title_text_right">{{i18n.Time}}</view>
		</view>
		<view class="tips" v-if="list.length === 0">{{i18n.No}}</view>
...
<script>
	...
	export default {
		...
		computed:{
			i18n(){
				return this.$t('message').mine.history
			}
		},
		methods: {
			toBack() {
