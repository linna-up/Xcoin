<template>
	<view class="usdt" :style="{width:windowWidth+'px',height:windowHeight+'px'}">
		...
		<view class="text">{{$t('message').mine.BindingUsdt.USDTAddress}}</view>
		<view class="addresstext">{{$t('message').mine.BindingUsdt.Address}}</view>
		<view class="address">
			<input type="text" :placeholder="$t('message').mine.BindingUsdt.AddressInput" v-model="address" />
		</view>
		<view class="texts">{{$t('message').mine.BindingUsdt.AddQr}}</view>
		<view class="add">
			<image src="../../static/img/mine/add.png" @click="getImg"></image>
			<image :src="img"></image>
		</view>
		<view class="btm" @click="updata">{{$t('message').mine.BindingUsdt.Save}}</view>
	</view>
</template>
		...
		methods: {
			...
			updata() {
				if (this.address === "" || this.address === null || this.address === undefined) {
					uni.showToast({
						title: this.$t('message').mine.BindingUsdt.usdtAddressInput,
						icon: "none"
					})
					return;
				}
