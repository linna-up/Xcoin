<template>
	<view class="addressmask" :style="{width:windowWidth+'px',height:windowHeight+'px'}">
		<view class="addressmask_box">
			...
			<view class="tell">{{$t('message').mine.AddAddress.AddAddressChild.TradePassword}}</view>
			<view class="yzbox">
				<view class="yzbox_left">
					<input type="password" v-model="ps" maxlength="6" :placeholder="$t('message').mine.AddAddress.AddAddressChild.InputTradePassword" />
				</view>
			</view>
		...
		methods: {
			...
			addAddress() {
				/* if (this.yzm === '') {
					...
					uni.showToast({
						title: this.$t('message').mine.AddAddress.AddAddressChild.InputTradePassword,
						icon: "none"
					})
					return;
				}
