<template>
	<view class="addAddress" :style="{width:windowWidth+'px',height:windowHeight+'px'}">
		...
		<view class="addAddress_title">{{$t('message').mine.AddAddress.AddUSDTAddress}}</view>
		<view class="address_text">{{$t('message').mine.AddAddress.Address}}</view>
		<view class="address_box">
			<input type="text" :placeholder="$t('message').mine.AddAddress.InputAddress" v-model="address" />
			<view class="address_box_right">
				<image src="../../static/img/collectcoin/scan.png" @click="collectCoin"></image>
			</view>
		</view>
		<view class="address_text">{{$t('message').mine.AddAddress.Note}}</view>
		<view class="address_box">
			<input type="text" v-model="bz" placeholder="USDT (ERC20) Address Name 1" />
		</view>
		<view class="btm" @click="addAddress">{{$t('message').mine.AddAddress.AddAddress}}</view>
	...
	export default {
		...
		methods: {
			...
			addAddress() {
				if(this.address=== '') {
					uni.showToast({
						title:this.$t('message').mine.AddAddress.InputSaveAddress,
						icon:"none"
					})
					return;
				}else if(this.bz=== '') {
					uni.showToast({
						title:this.$t('message').mine.AddAddress.InputAddress-m,
						icon:"none"
					})
					return;
				}
