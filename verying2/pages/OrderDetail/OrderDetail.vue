	...
	<view class="order" :style="{width:windowWidth+'px',height:(windowHeight)+'px'}">
		...
		<view class="content">
			<view class="box">
				<view class="box_left">
					<view class="box_left_box">
						<view class="box_left_box_left">¥ {{(Number(infos.unit_price)*Number(infos.num)).toFixed(4)}}</view>
						<view class="box_left_box_right" v-if="infos.status === '1'" @click="show">{{i18n.Appeal}}</view>
					</view>
					<view class="box_left_item">
						<view class="box_left_item_left">{{i18n.UnitPrice}}</view>
						<view class="box_left_item_right">¥ {{Number(infos.unit_price)}}</view>
					</view>
					<view class="box_left_item">
						<view class="box_left_item_left">{{i18n.Number}}</view>
			...
			<view v-if="infos.buy_type === 2">
				<view class="item">
					<view class="item_left">{{i18n.SellerNick}}</view>
					<view class="item_right">{{infos.buy_user_name}}</view>
				</view>
				<view class="item">
					<view class="item_left">{{i18n.BuyersName}}</view>
					<view class="item_right">{{infos.buy_real_name}}</view>
				</view>
			</view>
			<view v-if="infos.buy_type === 1">
				...
				<view class="item">
					<view class="item_left">{{i18n.SellerName}}</view>
					<view class="item_right">{{infos.buy_real_name}}</view>
				</view>
			</view>
			<view class="item">
				<view class="item_left">{{i18n.OrderNumber}}</view>
				<view class="item_right">{{infos.order_sn}}</view>
			</view>
			<view class="item">
				<view class="item_left">{{i18n.Time}}</view>
				<view class="item_right">{{infos.cdate}}</view>
			</view>
			<view class="shoukuan" v-show="pay.is_open_wx ==='1'" v-if="infos.status != '4'">
				<view class="shoukuan_left">{{i18n.Way}}</view>
			...
			<view class="shoukuan" v-show="pay.is_open_usdt === '1'" v-if="infos.status != '4'">
				...
				<view class="shoukuan_right" style="width: 50%!important;overflow: hidden;">
					<image style="margin-right: 8px;" :src="pay.alipay_img" @click="see(pay.usdt_img)"></image>
					<image src="../../static/img/tran/USDT.png"></image>
					<view style="width: 60%!important;overflow: hidden;">{{pay.wallet_address}}</view>
		...
		<view class="btm" v-show="infos.status === '1'" @click="yl">{{i18n.Check}}</view>
		<view class="btm" v-show="infos.is_show_pay_up === 1" @click="upPingz">{{i18n.Upload}}</view>
		<view v-if="infos.buy_type === 1"  class="qd" v-show="(infos.status === '1') || (infos.status ==='2')">{{i18n.Wait}}</view>
		<view v-if="infos.buy_type === 2" class="qd" v-show="infos.can_allow === 1" @click="fx">{{i18n.Release}}</view>
...
<script>
	...
	export default {
		...
		methods: {
			...
			see(img) {
				uni.previewImage({
					urls: [img],
					longPressActions: {
						itemList: [this.$t('message').OrderDetail.Save],
			...
			copy(val) {
				uni.setClipboardData({
					...
					success: function() {
						uni.showToast({
							title: this.$t('message').OrderDetail.Copy,
							icon: "none"
						})
					}
				});
		...
		computed: {
			i18n(){
				return this.$t('message').OrderDetail
			},
			getType() {
