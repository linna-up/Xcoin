<template>
	<view class="mask" :style="{width:width+'px',height:height+'px'}">
		<view class="mask_box">
			...
			<view class="box">
				<textarea placeholder-style="color:#ffffff;" v-model="shens" :placeholder="i18n.Content" />
				<view class="box_btm">
			...
			<view class="maskBtm_text" @click="shensu">{{i18n.Submit}}</view>
			<view class="maskBtm_btm" @click="cancel">{{i18n.Cancel}}</view>
		</view>
	</view>
</template>
	...
	export default {
		...
		methods: {
			...
			shensu() {
				if(this.shens === "") {
					uni.showToast({
						title: this.$t('message').OrderDetail.ContentInput,
						icon:"none"
					})
					return;
				}
				...
				uni.request({
					...
					success: (res) => {
						if(res.data.code === "success") {
							uni.showToast({
								title: this.$t('message').OrderDetail.Success,
								icon:"none"
							})
						}else {
							uni.showToast({
