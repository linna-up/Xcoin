<template>
	<view class="top">
		...
		<view class="btm" :style="{height:(140 -top -44)+'px'}">
			...
			<view class="btm_right" @click="toTell" v-if="status === '1'">
				<image src="../../../static/img/ass/lx.png"></image>
				<view>{{$t('message').OrderDetail.Contact}}</view>
			</view>
			<view class="btm_right" @click="toTell" v-if="status === '2'">
				<image src="../../../static/img/ass/lx.png"></image>
				<view>{{$t('message').OrderDetail.Contact}}</view>
			</view>
			<view class="btm_right" @click="toTell" v-if="status === '3'">
				<image src="../../../static/img/ass/lx.png"></image>
				<view>{{$t('message').OrderDetail.Contact}}</view>
			</view>
		</view>
...
<script>
	export default {
		...
		methods: {
			...
			toTell() {
				...
				if(this.list.buy_type === 2) {
					uni.makePhoneCall({
					    phoneNumber: this.list.cuser_phone //仅为示例
					});
				}else if(this.list.buy_type === 1) {
					uni.makePhoneCall({
					    phoneNumber: this.list.buser_phone //仅为示例
					});
				}
			}
		},
		computed:{
			...
			        }
			    }
			    var result = ""+parseInt(theTime)+ this.$t('message').OrderDetail.S;
			    if(middle > 0) {
			        result = ""+parseInt(middle)+ this.$t('message').OrderDetail.Min +result;
			    }
			    if(hour> 0) {
			        result = ""+parseInt(hour)+ this.$t('message').OrderDetail.H +result;
			    }
			    return result;
			}
		}
