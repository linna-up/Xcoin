<template>
	<view class="pay" :style="{width:windowWidth+'px',height:windowHeight+'px'}">
		...
		<view class="register_item">
			<view class="register_item_left">{{i18n.Amount}}:</view>
			<input type="number" v-model="money" :placeholder="i18n.InputAmount" placeholder-style="color:#B2B2B2;font-size:14px;" />
		</view>
		<view class="register_item">
			<view class="register_item_left">{{i18n.Password}}:</view>
			<input type="password" v-model="ps" :placeholder="i18n.InputPassword" placeholder-style="color:#B2B2B2;font-size:14px;" />
		</view>
		<view class="text">
			<span>{{i18n.Balance}}: </span><span style="color: #166A9B;">{{keyong}} {{i18n.XCoin}}</span>
			<view class="btm" @click="submit">{{i18n.Submit}}</view>
		</view>
		<view class="init" v-if="show" :style="{width:windowWidth+'px',height:(windowHeight)+'px'}">
			<view class="progress" v-if="show">
				...
				<view class="progress_text" v-if="show">{{i18n.Paying}}...</view>
			</view>
		</view>
	</view>
</template>
	...
	export default {
		...
		methods: {
			...
			submit() {
				if (this.money === '') {
					uni.showToast({
						title: this.$t('message').Pay.EnterAmount,
						icon: "none"
					})
					return;
				} else if (this.ps === '') {
					uni.showToast({
						title: this.$t('message').Pay.EnterPassword,
						icon: "none"
					})
					return;
				}
		...
		onBackPress(e) {
			if (e.from == 'backbutton') {
				if(this.show) {
					uni.showToast({
						title: this.$t('message').Pay.Transfer,
						icon: "none"
					})
					return true;
				}else {
