<template>
	<view class="pro" :style="{width:windowWidth+'px',height:(windowHeight)+'px'}">
		<ProNav></ProNav>
		<textarea :placeholder="i18n.Content" v-model="content"></textarea>
		<view class="tell">
			<view style="font-size: 16px;">{{i18n.Contact}}</view>
		...
		<view class="btn" @click="upProblem">{{i18n.Submit}}</view>
	</view>
</template>
	...
	export default {
		...
		methods: {
			...
			upProblem() {
				if(this.content === "") {
					uni.showToast({
						title:this.$t('meesage').Problem.Input,
						icon:"none"
					})
					return;
				}
