<template>
	<view class="coincharging" :style="{width:windowWidth+'px',height:(windowHeight)+'px'}">
		...
		<view class="content" :style="{width:windowWidth+'px',height:(windowHeight-top-46)+'px'}">
			<view class="content_title">{{i18n.Recharge}}</view>
			...
			<view class="content_box">
				...
				<view class="save" @click="capture">{{i18n.SaveQR}}</view>
				<view class="text">{{i18n.RechargeAddress}}</view>
				<view class="to"><p>{{val}}</p></view>
				<view class="copy" @click="copy">{{i18n.Copy}}</view>
			</view>
			<view class="Explain">
				<p>{{i18n.Text1}}</p>
				<p>{{i18n.Text2}}</p>
...
<script>
	...
	export default {
		...
		computed:{
			i18n(){
				return this.$t('message').mine.CoinCharging
			}
		},
		methods: {
			capture() {
					...
					bitmap.save("_doc/a.jpg", {}, function(i) {
						...
						uni.saveImageToPhotosAlbum({
							...
							success: function() {
								...
								uni.showToast({
									title: this.$t('message').mine.CoinCharging.SaveSucceed,
									mask: false,
									duration: 1500,
									icon: "none"
								});
