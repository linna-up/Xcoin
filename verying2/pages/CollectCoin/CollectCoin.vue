<template>
	<view class="collectcoin" :style="{width:windowWidth+'px',height:windowHeight+'px'}">
		...
		<view class="content_title">{{i18n.Withdrawal}}</view>
		<view class="content_type" v-if="type === 0">
			<view class="content_type_img">
				<image src="../../static/img/home/USDT.png"></image>
			</view>
			<view class="content_type_text">USDT(ERC20)</view>
		</view>
		<view class="content_type" v-else-if="type === 1">
			<view class="content_type_img">
				<image src="../../static/img/index/xev.png"></image>
			</view>
			<view class="content_type_text">{{i18n.XCoin}}</view>
		</view>
		<view class="address_text">{{i18n.WithdrawalAddress}}</view>
		<view class="address">
			<input type="text" v-model="address" :placeholder="i18n.CopyInput" />
		...
		<view class="address_text">{{i18n.WithdrawalAmount}}</view>
		<view class="num">
			<input type="number" class="address_input" v-model="num" :placeholder="i18n.Mini" @blur="BiLi" />
			<view class="num_right">
				<view class="num_right_left">
					<span v-if="type === 0">USDT</span>
					<span v-else-if="type === 1">{{i18n.XCoin}}</span>
					<image v-if="type === 0" src="../../static/img/home/USDT.png"></image>
					<image v-else-if="type === 1" src="../../static/img/logo/XEV.png"></image>
				</view>
				<view class="address_right_center"><span></span></view>
		...
		<view class="available" v-if="type === 0">{{i18n.Usable}} {{all}} USDT</view>
		<view class="available" v-else-if="type === 1">{{i18n.Usable}} {{all}} {{i18n.XCoin}}</view>
		<view class="address_text">{{i18n.ServiceCharge}}</view>
		<view class="service">
		...
		<view class="service">
			<view class="service_lefts">{{i18n.AccountNumber}}</view>
			<view class="service_rights">
				<span v-if="type === 0">{{dz}}USDT</span>
				<span v-else-if="type === 1">{{dz}}{{i18n.XCoin}}</span>
				<image v-if="type === 0" src="../../static/img/home/USDT.png"></image>
				<image v-else-if="type === 1" src="../../static/img/logo/XEV.png"></image>
			</view>
		</view>
		<view class="btm" @click="tiBi">{{i18n.Withdrawal}}</view>
	</view>
</template>
	...
	export default {
		data() {
			return {
				...
				list: ['USDT', this.$t('message').mine.CollectCoin.XCoin],
				address: null,
				num: null,
				/* apply_type:"XEV", */
				type: 0,
		...
		computed: {
			...
			i18n(){
				return this.$t('message').mine.CollectCoin
			}
		},
		watch: {
			fee(newVal) {
