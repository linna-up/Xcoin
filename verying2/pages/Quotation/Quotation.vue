		...
		<mescroll-body class="scroll" ref="mescrollRef" @init="mescrollInit" @down="downCallback" @up="upCallback" :down="downOption"
		 :up="upOption">
			<view class="title" :style="{paddingTop:top+'px'}">
				...
				<view class="title_center">{{i18n.Market}}</view>
				<view class="title_right">
					<!-- <image src="../../static/img/quotation/search.png"></image> -->
					<view class="title_right_time" @click="toJump">{{i18n.ServiceCenter}}</view>
				</view>
			</view>
			<view class="box_title">
				<view class="box_title_left"><span>{{i18n.Name }}</span><span class="bg"></span></view>
				<view class="box_title_center"><span>{{i18n.New}}</span><span class="bg"></span></view>
				<view class="box_title_right"><span>{{i18n.Applies}}</span><span class="bg"></span></view>
			</view>
...
<script>
	...
	export default {
		...
		data() {
			return {
				...
				upOption: {
					...
					empty: {
						tip: this.$t('message').Quotation.NoData
					}
				}
			}
		},
		...
		computed:{
			i18n(){
				return this.$t('message').Quotation
			}
		},
		methods: {
			/*下拉刷新的回调*/
