	...
	export default {
		data() {
			return {
				...
				thisIndex:0,
				list:[
					'100' + this.$t('message').PetroleumEtc.CNY,
					'200' + this.$t('message').PetroleumEtc.CNY,
					'500' + this.$t('message').PetroleumEtc.CNY,
					'1000' + this.$t('message').PetroleumEtc.CNY,
					this.$t('message').PetroleumEtc.More,
				],
				isShow:false
			}
		},
		...
		methods: {
			...
			changeIndex(index) {
				if(index === 4) {
					uni.showToast({
						title: this.$t('message').PetroleumEtc.Increase,
						duration:1500,
						icon:'none'
					})
					return;
			...
			toFlow() {
				uni.showToast({
					title:this.$t('message').PetroleumEtc.Unopen,
					duration:1500,
					icon:'none'
				})
			}
