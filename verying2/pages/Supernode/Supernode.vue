<template>
	<view class="node" :style="{width:windowWidth+'px',height:(windowHeight)+'px'}">
		...
		<view class="yj">
			<view class="yj_box"><span>{{i18n.SuperNode}}</span>
		...
		<view class="yj">
			<view class="yj_box"><span>{{i18n.For}}</span>
				<!-- <image src="../../static/img/team/node.png"></image> -->
			</view>
		</view>
		<view class="team">
			<view class="team_box">
				<view class="team_box_left"><input :placeholder="i18n.InputPhone" v-model="tell" @input="searchPhone" /></view>
		...
		<view class="box" v-if="list != null" >
			...
			<view class="box_item box_items"><view @click="toStart">{{i18n.Open}}</view></view>
		</view>
	</view>
</template>
		...
		data() {
			return {
				...
				llist: [/* {
					...
					{
						value: 100,
						text: this.$t('message').SuperNode.Deposit
					}
				],
				list:null,
				tell:"",
