<template>
	<view class="live"  :style="{width:windowWidth+'px',height:windowHeight+'px'}">
		...
		<view class="text">{{i18n.Methods}}</view>
		<Item v-show="allList.is_open_alipay != null" :list="zfb" ></Item><!-- 支付宝 -->
		<Item v-show="allList.is_open_wx != null" :list="wx" ></Item><!-- 微信 -->
		<Item v-show="allList.is_open_bank != null" :list="yl"></Item><!-- 银行卡 -->
		<UItem v-show="allList.is_open_usdt != null" :list="usdt" ></UItem><!-- usdt -->
		<view class="btm" @click="toBinding">{{i18n.Binding}}</view>
	</view>
</template>
