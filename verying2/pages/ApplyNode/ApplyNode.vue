<template>
	<view class="apply" :style="{width:windowWidth+'px',height:windowHeight+'px'}">
		<view class="node_title" :style="{paddingTop:top+'px'}">
			...
			{{$t('message').mine.ApplyNode.ApplyNode}}
		</view>
		<view class="item" style="margin-top: 30px;">
			<view class="item_left">{{$t('message').mine.ApplyNode.CurrentLevel}}:</view>
			<view class="item_right">
				<image :src="getImg"></image>
				<view style="color: #0067A2;">{{$t('message').mine.ApplyNode.User}}</view>
			</view>
		</view>
		<view class="item">
			<view class="item_left">{{$t('message').mine.ApplyNode.ApplyLevel}}:</view>
			<view class="item_right">
				<view>{{$t('message').mine.ApplyNode.ServiceNode}}</view>
			</view>
		</view>
		<view class="item">
			<view class="item_left">{{$t('message').mine.ApplyNode.NodeAmount}}:</view>
			<view class="item_right">
				<view style="color: #0067A2;">0.00<!-- {{S2_fee}} -->USDT</view>
			</view>
		</view>
		<view class="ye">{{$t('message').mine.ApplyNode.AccountBalance}}:<span>{{usdt}}USDT</span></view>
		<button class="btm" :disabled="active" @click="applyNode">{{$t('message').mine.ApplyNode.Submit}}</button>
	</view>
</template>
