<template>
	<view class="panel">
		...
		<view class="panel_box">
			<view class="status">
				<span class="status_title">{{i18n.State}}</span>
			...
			<view class="isFalse" v-if="getShow === false">
				<view class="create" @click="create" :style="{marginTop:marginTop}">{{i18n.Create}}</view>
			</view>
			<view class="isFalse" v-else>
				<view class="create" :style="{marginTop:marginTop}" @click="tellSuperToCreate">{{i18n.Check}}</view>
				<view class="create" :style="{marginTop:marginTop}" @click="tellSuperToCreate">{{i18n.Back}}</view>
			</view>
		</view>
		<!-- 开关 -->
		<!-- <view class="switch">
...
<script>
	...
	export default {
		...
		methods: {
			tellCMask() {
				if (this.contract_status === 1) {
					uni.showToast({
						title: this.$t('message').mine.contract.Lock,
						icon: "none"
					})
					return;
				}
		...
		computed: {
			i18n(){
				return this.$t('message').mine.contract
			},
			getShow() {
				if (this.contract_status === 1) {
					return true;
