<template>
	<view class="panel">
		...
		<view class="money">
			<span>{{i18n.Time}}:</span>
			<span>{{list.line_time}}{{i18n.Day}}</span>
		</view>
		<view class="gainbox">
			<span class="gainbox_left">{{i18n.Pending}}:</span>
			<span class="gainbox_item">{{list.no_amount}}</span>
			<span class="gainbox_item">
				<view class="gain" @click="getSy">{{i18n.Receive}}</view>
			</span>
		</view>
		<view class="gainbox">
			<span class="gainbox_left">{{i18n.ApplyBack}}:</span>
			<span class="gainbox_item"></span>
			<span class="gainbox_item">
				<view class="apply" @click="getTo_quit">{{i18n.Apply}}</view>
			</span>
		</view>
...
<script>
	...
	export default {
		...
		methods: {
			...
			getTo_quit() {
				if(Number(this.list.no_amount)>0) {
					uni.showToast({
						title:this.$t('message').mine.contractualing.Earnings,
						icon:"none"
					})
					return;
				}
		...
		computed: {
			i18n(){
				return this.$t('message').mine.contractualing
			}
		}
	}
</script>
