<template>
	<view class="transaction" :style="{width:width+'px',height:(height)+'px'}">
		...
		<view class="tabBox" style="position: fixed;z-index: 99990;background-color: #FFFEFE;" :style="{top:(44+top)+'px'}">
			<view class="tabBoxc">
				<view class="usdt" @click="ItemClick(0)"><span :class="{active:activeIndex === 0}">USDT</span></view>
				<view class="xev" @click="ItemClick(1)"><span :class="{active:activeIndex === 1}">{{i18n.XCoin}}</span></view>
				<view v-if="is_store === '1'" class="onekeybuy" @click="showAllMask">{{i18n.Release}}</view>
			</view>
		</view>
	...
	export default {
		...
		data() {
			return {
				...
				activeIndex: 0,
				coinType: [
					this.$t('message').transaction.Coin,
					 this.$t('message').transaction.Fiat,
					 ],
				maskShow: false,
				allShow: false,
				set: [],
				showType: {
					coinType: 0,
					type: 0,
					title: this.$t('message').transaction.Buy,
					img: "",
					price: 1.00,
					all: 0
				},
				...
				upOption: {
					...
					empty: {
						tip: this.$t('message').transaction.Nodata
					}
				},
				coinIndex: 0,
				usdtBuyBigid: 0,
			...
			uni.$on('ShowMask', (data) => {
				...
				if (data.list.coinType === 0) {
					...
					if (data.list.current === 0) {
						this.showType.title = this.$t('message').transaction.Buy;
						this.showType.all = 0;
					} else if (data.list.current === 1) {
						this.showType.all = 1;
						this.showType.title = this.$t('message').transaction.Sell;
					}
					this.showType.price = 1.00;
				} else if (data.list.coinType === 1) {
					this.showType.coinType = 0;
					this.showType.type = this.$t('message').transaction.XCoin;
		...
		methods: {
			...
			downCallback() {
				this.getJiao_Yi();
				uni.showToast({
					title: this.$t('message').transaction.Refresh,
					duration: 1000,
					icon: "none"
				})
			...
			upCallback(page) {
				...
				this.getJiao_Yis();
				uni.showToast({
					title: this.$t('message').transaction.Load,
					duration: 1000,
					icon: "none"
				})
				setTimeout(() => {
