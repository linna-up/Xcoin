		...
		<view class="mask_box">
			<view class="mask_title">
				<view class="mask_title_left">
					...
					<view class="mask_title_left_btm">{{i18n.Unit}} ¥{{AllList.unit_price}}</view>
				</view>
			...
			<view class="maskInput">
				...
				<view class="maskInput_right">
					<view class="maskInput_right_item" v-if="showType.all === 0" @click="allBuySell">{{i18n.AllBuy}}</view>
					<view class="maskInput_right_item" v-else @click="allBuySell">{{i18n.All}}</view>
				</view>
			</view>
			<view class="maskText">{{i18n.Norm}}: {{AllList.s_money}} - {{AllList.l_money}}</view>
			<view class="maskTextRight">{{i18n.DealsNumber}}: {{jiao_shuliang}}{{showType.type}}</view>
			<view class="maskNum">
				<view class="maskNum_text">{{i18n.Real}}</view>
				<view class="maskNum_right">¥{{Number(AllList.unit_price)*Number(num)}}</view>
			</view>
			<view class="maskBtm">
				<view class="maskBtm_text">{{time}}{{i18n.Cancel}}</view>
				<view class="maskBtm_btm" @click="confirmBuySell">{{i18n.Place}}</view>
			</view>
			<view class="chose">
...
<script>
	...
	export default {
		...
		computed: {
			...
			getTips() {
				return this.$t('message').transaction.Input +this.showType.title+this.$t('message').transaction.Number;
			}/* ,
			getImg(){
